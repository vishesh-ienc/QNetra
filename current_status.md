...
└───────────────────────────────────┬─────────────────────────────────────────────┘
                                    ...
                                    ▼
┌─────────────────────────────────────────────────────────────────────────────────┐
│ 2. Canonical Normalization & Aggregation Layer (core.normalization)   [PLANNED] │
│    - Algorithm canonicalization & alias resolution                              │
│    - Multi-scanner finding deduplication & entity resolution                   │
│    - Emits canonical CryptoAsset records                                        │
└───────────────────────────────────┬─────────────────────────────────────────────┘
                                    │ Emits Canonical CryptoAsset
                                    ▼
┌─────────────────────────────────────────────────────────────────────────────────┐
│ 3. CBOM Generation & Classification Layer (core.cbom / classification) [PLANNED]│
│    - CycloneDX 1.6+ JSON & XML CBOM Serializer                                  │
│    - Functional & Quantum Threat Classification (Shor / Grover)                 │
└───────────────────────────────────┬─────────────────────────────────────────────┘
                                    │
                                    ▼
┌─────────────────────────────────────────────────────────────────────────────────┐
│ 4. Downstream Intelligence Engines (core.risk / mosca / recommendations)[PLANNED]│
...
└───────────────────────────────────┬─────────────────────────────────────────────┘
                                    ...
                                    ▼
┌─────────────────────────────────────────────────────────────────────────────────┐
│ 5. API Gateway & Interactive UI Dashboard (backend / frontend)        [PLANNED] │
...
| :--- | :--- | :--- | :--- |
| **Phase 2** | `core/normalization/` | Algorithm name canonicalization, alias resolution, multi-scanner deduplication, and translation to canonical `CryptoAsset` models. | 🕒 **Immediate Next** |
| **Phase 2** | `core/classification/` | Categorize cryptographic assets by primitive type (PKC, Symmetric, Hash, MAC, KDF) and assign quantum threat vectors (Shor vs. Grover). | 🕒 Planned |
| **Phase 2** | `core/cbom_generator/` | CycloneDX 1.6+ JSON & XML CBOM serialization conforming to official cryptography extensions. | 🕒 Planned |
...
scanners/registry (Algorithms, APIs, Libraries, etc.)   186        16         91%
...
## 7. Immediate Next Development Actions (Phase 2 Launch)

1. **Create Normalization Layer (`core/normalization/`):**
   * Implement `Normalizer` class to transform `List[RawFinding]` into canonical `List[CryptoAsset]`.
   * Standardize algorithm names (e.g. `AES_256_GCM`, `aes256`, `AES/GCM/NoPadding` ➔ `AES-256-GCM`).
   * Perform intelligent finding deduplication across multi-pass scanners.
2. **Implement Classification Engine (`core/classification/`):**
   * Enrich `CryptoAsset` instances with quantum vulnerability classes (Shor-vulnerable vs. Grover-impacted).
   * Calculate effective quantum security bits.
3. **Implement CBOM Generator (`core/cbom_generator/`):**
   * Build CycloneDX 1.6+ JSON & XML CBOM serializer.
   * Add schema validation test fixtures.
4. **Author Phase 2 Unit Tests (`tests/test_core/`):**
   * Normalization accuracy tests, algorithm alias resolution tests, CBOM schema compliance tests.
...
> 1. Update this document (`current_status.md`) with the latest test results, module status, and metrics.
> 2. Synchronize [`PROJECT_CONTEXT.md`](file:///c:/Users/VISHESH/Desktop/QNetra/PROJECT_CONTEXT.md) and [`docs/07_PROGRESS.md`](file:///c:/Users/VISHESH/Desktop/QNetra/docs/07_PROGRESS.md).
> 3. If new architectural choices or rules were introduced, log them in [`docs/08_DECISIONS_AND_LOG.md`](file:///c:/Users/VISHESH/Desktop/QNetra/docs/08_DECISIONS_AND_LOG.md) and [`PROJECT_RULES.md`](file:///c:/Users/VISHESH/Desktop/QNetra/PROJECT_RULES.md).

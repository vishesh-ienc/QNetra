    ...
    <>
      ...
      <Section eyebrow="Export" title="Formats" lede="What can be produced from this scan today.">
        <div className={styles.exports}>
          ...
          <Unavailable
            label="CycloneDX 1.6 XML — requires the API service"
            reason={
              <>
                The XML serialiser lives in{' '}
                <span className="mono">core.cbom_generator</span> and is reached through{' '}
                <span className="mono">GET /scans/{'{id}'}/cbom/export?format=xml</span>.
                {API_MODE === 'mock'
                  ? ' That endpoint is not available in this session because the QNetra API service is not running.'
                  : ' The endpoint returned an error or is not yet implemented on this instance.'}{' '}
                The interface will not re-serialise the document itself — a CBOM generated by the
                frontend would not be the CBOM the engine produced.
              </>
            }
          />
        </div>
...
function downloadCbom(document_: unknown, scanId: string): void {
  ...
  link.href = url;
  link.download = `qnetra-cbom-${scanId}.json`;
  link.click();
